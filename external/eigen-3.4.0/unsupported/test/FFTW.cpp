...
void test_complex2d()
{
    ...
    FFT<T> fft;
    Eigen::Matrix<Complex,nrows,ncols> external,src2,dst,dst2;

    external = Eigen::Matrix<Complex,nrows,ncols>::Random();
    //external =  Eigen::Matrix<Complex,nrows,ncols>::Identity();

    for (int k=0;k<ncols;k++) {
        Eigen::Matrix<Complex,nrows,1> tmpOut;
        fft.fwd( tmpOut,external.col(k) );
        dst2.col(k) = tmpOut;
    }
    ...
    fft.fwd2(dst.data(),external.data(),ncols,nrows);
    fft.inv2(src2.data(),dst.data(),ncols,nrows);
    VERIFY( (external-src2).norm() < test_precision<T>() );
