 ...
 private:
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE void Run(
    ...
    if (kind == StridedLinearBufferCopy::Kind::Linear) {
      // ******************************************************************** //
      // Linear copy from `external` to `dst`.
      ...
      // Fill `dst` with value at `*external`.
...
// -------------------------------------------------------------------------- //
// TensorBlockIO copies data from `external` tensor block, to the `dst` tensor block.
// It's possible to specify external->dst dimension mapping for the copy operation.
// Dimensions of `dst` specify how many elements have to be copied, for the
// `external` we need to know only stride to navigate through source memory buffer.
...
class TensorBlockIO {
 ...
 public:
  ...
  // Copies data to `dst` from `external`, using provided dimensions mapping:
  //
  ...
  static EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE IndexType Copy(
    ...
    // Copy single scalar value from `external` to `dst`.
    if (NumDims == 0) {
      *(dst.data + dst.offset) = *(src.data + src.offset);
      return 1;
    }

    // Both `dst` and `external` must have contiguous innermost dimension. We also
    ...
    // the external. If dimensions are reordered, we might end up reading data from
    // the external with `stride != 1`.
    //
    ...
    // If all dimensions are of size 1, just copy a scalar from `external` to `dst`.
    ...
    // Dimension in the external that corresponds to the dst innermost dimension.
    ...
    // `external` memory, so we can do less linear copy calls.
    ...
    // Setup strides to read data from `external` and write to `dst`.
    ...
    // Iterate copying data from external to dst.
...
#undef COPY_INNER_DIM
  ...
  // Copy from `external` to `dst` with an identity external->dst dimension map. Returns
...
// `target->external` dimension map (see definition above).
//
...
// where `external` is a tensor expression. Explore if it is possible to rewrite IO
